...
namespace PeakTrollMod
{
    internal sealed class TrollUIManager
    {
        ...
        private readonly string[] _tabs = { "⌂  Home", "♟  Player", "▣  Spawning", "✦  Mirage", "♪  Audio", "●  Appearance", "☠  Enemies", "▲  World", "◈  Chaos Mode", "⚙  Settings" };
        ...
        public void Draw()
        {
            ...
            GUI.color = Color.white; DrawRect(new Rect(0, 0, Screen.width, Screen.height), new Color(.015f, .03f, .06f, .72f));
            GUI.matrix = Matrix4x4.TRS(new Vector3(x, y, 0f), Quaternion.identity, new Vector3(scale, scale, 1f));
            DrawPanel(new Rect(10, 10, 1260, 830), new Color(.025f, .07f, .12f, Mathf.Clamp(_plugin.Settings.Transparency.Value, .55f, 1f)), new Color(.23f, .45f, .62f, 1f), 2f);
        ...
        {
            GUI.Label(new Rect(42, 25, 285, 72), "PEAK", _logo);
            GUIStyle purple = new GUIStyle(_logo); purple.fontSize = 45; purple.normal.textColor = new Color(.60f, .42f, 1f); GUI.Label(new Rect(300, 32, 390, 60), "TROLL MOD", purple);
            GUI.Label(new Rect(304, 86, 430, 25), "SAME FRIENDS. A HIGHER LEVEL OF CHAOS.", _small);
            GUIStyle quote = new GUIStyle(_small); quote.fontSize = 16; quote.alignment = TextAnchor.MiddleCenter; quote.normal.textColor = new Color(.7f, .77f, .9f); GUI.Label(new Rect(830, 38, 260, 60), "“IT'S JUST A PRANK...\nRIGHT?”", quote);
            if (GUI.Button(new Rect(1190, 26, 48, 40), "×", _button)) ForceClose();
            Line(new Rect(210, 118, 1040, 1), new Color(.2f, .42f, .58f, .7f));
        }
        ...
        private void DrawContent()
        {
            ...
            GUI.Label(new Rect(244, 136, 520, 43), _tabNames[_tab], _title);
            GUI.Label(new Rect(246, 176, 610, 25), SubtitleForTab(), _small);
            if (_tab != 0 && _tab != 9)
            {
                GUI.Label(new Rect(820, 146, 115, 30), "Target Player:", _label);
                if (GUI.Button(new Rect(932, 139, 202, 42), CurrentTargetName() + "  ▾", _button)) CycleTarget();
                if (GUI.Button(new Rect(1138, 139, 42, 42), "R", _button)) SelectRandomTarget();
                if (GUI.Button(new Rect(1184, 139, 44, 42), "ALL", _button)) _targetAll = true;
            }
        ...
        private void DrawHome(Rect area)
        {
            ...
            {
                GUILayout.BeginHorizontal(); if (GUILayout.Button("STOP CHAOS", _danger, GUILayout.Height(42))) { _plugin.Chaos.Stop(); _plugin.Chaos.StopCombo(); Say("Chaos and combo runner stopped."); } if (GUILayout.Button("CLEAR MIRAGES", _button, GUILayout.Height(42))) { _plugin.Mirages.ClearAll(); Say("All local mirages cleared."); } if (GUILayout.Button("CLEAR TROLL SPAWNS", _button, GUILayout.Height(42))) ClearTrollSpawns(); GUILayout.EndHorizontal();
                GUILayout.Space(12); if (GUILayout.Button("RESET ALL TROLL EFFECTS", _danger, GUILayout.Height(48))) EmergencyReset();
        ...
        {
            Card(new Rect(238, 212, 310, 250), "♥  Health / Status", new Color(1f, .33f, .38f), delegate
            {
                Badge(PermissionKind.Anyone); GUILayout.Label("Knockout / Eliminate are native and mod-free.", _small); if (CapabilityButton("KNOCK OUT", FeatureCapability.Knockout, _danger, 34)) Owner(NetCommand.Knockout, new object[0]);
                if (CapabilityButton(_confirmEliminate ? "CONFIRM ELIMINATE" : "ELIMINATE", FeatureCapability.Eliminate, _danger, 34)) { if (!_plugin.Settings.ConfirmDestructive.Value || _confirmEliminate) { Owner(NetCommand.Eliminate, new object[0]); _confirmEliminate = false; } else { _confirmEliminate = true; Say("Click Eliminate again to confirm."); } }
                GUILayout.BeginHorizontal(); if (GUILayout.Button(((CharacterAfflictions.STATUSTYPE)_statusIndex).ToString(), _button, GUILayout.Width(150))) _statusIndex = (_statusIndex + 1) % 15; if (CapabilityButton("APPLY", FeatureCapability.StatusEffects, _button, 30)) Owner(NetCommand.Status, new object[] { _statusIndex, _statusAmount }); GUILayout.EndHorizontal();
                _statusAmount = GUILayout.HorizontalSlider(_statusAmount, .01f, .5f); GUILayout.Label("Intensity " + _statusAmount.ToString("0.00"), _small);
                GUILayout.Label(_plugin.Players.IsHost&&_statusIndex!=7&&_statusIndex!=9&&_statusIndex!=12?"🔒 Host-native status; target mod not required.":"🟡 Status requires the selected target's mod.",_small);if(GUILayout.Button("CLEAR MOD STATUSES",_button,GUILayout.Height(26)))ClearStatuses();
            });
            Card(new Rect(558, 212, 330, 250), "↗  Movement", new Color(.23f, .72f, 1f), delegate
            {
                Badge(PermissionKind.Anyone); GUILayout.Label("Launch Strength  " + _launchStrength.ToString("0"), _label); _launchStrength = GUILayout.HorizontalSlider(_launchStrength, 5f, 65f);
                ...
                GUILayout.Label("🟢 Anyone — native PEAK RPC; target needs no mod", _small);
            ...
            {
                Badge(PermissionKind.EveryoneNeedsMod); GUILayout.Label("Renderer state is cached before modification.", _small);
                if (CapabilityButton("INVISIBLE ON", FeatureCapability.Visibility, _button, 38)) Broadcast(NetCommand.Visibility, new object[] { false });
                if (CapabilityButton("INVISIBLE OFF / RESTORE", FeatureCapability.Visibility, _button, 38)) Broadcast(NetCommand.Visibility, new object[] { true });
                Unsupported("Third person / nametags / fake AFK", "No verified stable runtime API.");
            });
            Card(new Rect(238, 475, 310, 215), "⚡  Speed", new Color(.26f, 1f, .54f), delegate
            {
                Badge(PermissionKind.EveryoneNeedsMod); GUILayout.Label("Multiplier  " + _speed.ToString("0.00") + "x", _label); _speed = GUILayout.HorizontalSlider(_speed, .25f, 3f);
                if (CapabilityButton("APPLY SPEED", FeatureCapability.Speed, _success, 38)) Owner(NetCommand.Speed, new object[] { _speed });
                if (GUILayout.Button("RESET TO 1.0x", _button, GUILayout.Height(34))) Owner(NetCommand.Speed, new object[] { 1f });
            });
            Card(new Rect(558, 475, 330, 215), "⌖  Teleport", new Color(.61f, .4f, 1f), delegate
            {
                Badge(PermissionKind.Anyone);
                GUILayout.Label("Native PEAK warp RPC; no host or target mod required.", _small);
                if (CapabilityButton("TELEPORT MYSELF TO TARGET", FeatureCapability.Teleport, _success, 32)) TeleportSelfToTarget();
                if (GUILayout.Button("TELEPORT TO ME", _button, GUILayout.Height(34)) && _plugin.Players.Local != null) Teleport(_plugin.Players.Local.Character.Center);
                GUILayout.BeginHorizontal(); if (GUILayout.Button("TO START", _button)) { Vector3 p; if (_plugin.Actions.TryGetStartEnd(false,out p)) Teleport(p); else Say("Start reference unavailable."); } if (GUILayout.Button("TO END", _button)) { Vector3 p; if (_plugin.Actions.TryGetStartEnd(true,out p)) Teleport(p); else Say("End reference unavailable."); } GUILayout.EndHorizontal();
                if (GUILayout.Button("RANDOM NEAR PLAYER", _button)) { PlayerEntry r=_plugin.Players.Random(true); if(r!=null) Teleport(r.Character.Center + UnityEngine.Random.insideUnitSphere * 3f); }
                GUILayout.Label("Safe ground and geometry checks are always enabled.", _small);
            });
            Card(new Rect(898, 475, 330, 215), "↻  Item / Reset", new Color(1f, .65f, .22f), delegate
            {
                Badge(PermissionKind.Anyone); _itemSearch=GUILayout.TextField(_itemSearch, _button, GUILayout.Height(30)); string item = CurrentItemName(); if (GUILayout.Button(item + "  ▾", _button, GUILayout.Height(34))) CycleItem();
                if (CapabilityButton("SPAWN IN HANDS", FeatureCapability.GiveItem, _button, 36)) Owner(NetCommand.GiveItem, new object[] { item });
                if (GUILayout.Button("FIND MANDRAKE", _button)) SelectItemContaining("Mandrake");
                GUILayout.Label("Vanilla request is routed to the Photon master; target needs no mod.", _small);
                if (GUILayout.Button("RESET PLAYER", _success, GUILayout.Height(36))) Broadcast(NetCommand.Reset, new object[0]);
            });
        }

        private void DrawSpawning(Rect area)
        {
            Card(new Rect(238, 220, 315, 270), "☠  Scoutmaster Ambush", new Color(1f, .3f, .35f), delegate { Badge(_plugin.Players.IsHost?PermissionKind.HostOnly:PermissionKind.EveryoneNeedsMod); Distance(); GUI.enabled=_plugin.Network.HostSupportsRequests; if (GUILayout.Button("REQUEST SCOUTMASTER", _button, GUILayout.Height(42))) Say(_plugin.Network.RequestHostSpawn(NetCommand.HostSpawnScoutmaster,Target(),_spawnDistance)); GUI.enabled=true; GUILayout.Label(_plugin.Players.IsHost?"Executed with local host authority.":"A compatible host executes the verified room spawn.", _small); });
            Card(new Rect(565, 220, 315, 270), "♟  Mushroom Zombie", new Color(.45f, .8f, .35f), delegate { Badge(_plugin.Players.IsHost?PermissionKind.HostOnly:PermissionKind.EveryoneNeedsMod); Distance(); GUI.enabled=_plugin.Network.HostSupportsRequests; if (GUILayout.Button("REQUEST ZOMBIE", _button, GUILayout.Height(42))) Say(_plugin.Network.RequestHostSpawn(NetCommand.HostSpawnZombie,Target(),_spawnDistance)); GUI.enabled=true; GUILayout.Label("A compatible host resolves the vanilla spawner prefab.", _small); });
        ...
        private void DrawMirage(Rect area)
        {
            Card(new Rect(238, 218, 315, 265), "✦  Ping Placement", new Color(.35f, .65f, 1f), delegate
            {
                ...
                _multiPlace = GUILayout.Toggle(_multiPlace, "Multi-place"); if (GUILayout.Button("CANCEL PLACEMENT", _danger)) _plugin.PingPlacement.Cancel(); GUILayout.Label(_plugin.PingPlacement.Active ? "Aim and use the normal ping input." : "Placement inactive.", _small);
            });
            Card(new Rect(565, 218, 315, 265), "♟  Mirage Scout", new Color(.63f, .42f, 1f), delegate
            {
                ...
                if (GUILayout.Button("STAND & STARE BEHIND", _button, GUILayout.Height(38))) CreateScout(MirageBehavior.StandAndStare);
                if (GUILayout.Button("FOLLOW AT DISTANCE", _button, GUILayout.Height(38))) CreateScout(MirageBehavior.FollowAtDistance);
                if (GUILayout.Button("RUN AWAY", _button, GUILayout.Height(38))) CreateScout(MirageBehavior.RunAway);
            });
            Card(new Rect(892, 218, 336, 265), "☠  Fake Enemy", new Color(1f, .35f, .42f), delegate
            {
                Badge(PermissionKind.EveryoneNeedsMod); if (GUILayout.Button(((FakeEnemyKind)_enemyIndex).ToString() + "  ▾", _button)) _enemyIndex = (_enemyIndex + 1) % 3; Lifetime();
        ...
        private void DrawAudio(Rect area)
        {
            ...
            {
                Badge(PermissionKind.EveryoneNeedsMod); GUILayout.Label("Runtime clip: " + CurrentSoundName(), _label); if (GUILayout.Button("NEXT DISCOVERED CLIP", _button)) CycleSound();
                GUILayout.Label("Volume " + _volume.ToString("0.00"), _label); _volume = GUILayout.HorizontalSlider(_volume, 0f, 1f);
                GUILayout.Label("Position is behind the selected player.", _small); if (GUILayout.Button("PLAY 3D SOUND", _button, GUILayout.Height(44))) { PlayerEntry t=Target(); if(t!=null){Vector3 p=t.Character.Center-t.Character.data.lookDirection_Flat*3f; Owner(NetCommand.FakeAudio,new object[]{CurrentSoundName(),p.x,p.y,p.z,_volume});} }
                if(GUILayout.Button("PLAY LOCALLY NEAR TARGET",_success,GUILayout.Height(36))){PlayerEntry t=Target();if(t!=null){Vector3 p=t.Character.Center-t.Character.data.lookDirection_Flat*3f;Say(_plugin.Audio.PlayDiscovered(CurrentSoundName(),p,_volume,t.ActorNumber));}}
        ...
        private void DrawAppearance(Rect area)
        {
            Card(new Rect(238,220,480,330),"●  Local Appearance Swap",new Color(.63f,.42f,1f),delegate
            {
                ...
                if(GUILayout.Button("RESTORE SELECTED",_success,GUILayout.Height(38)))RestoreAppearances();
            });
        ...
        private void DrawWorld(Rect area)
        {
            Card(new Rect(238,220,480,390),"✹  Dynamite Shower",new Color(1f,.42f,.2f),delegate
            {
                ...
                if(GUILayout.Button("CANCEL + CLEAR MY DYNAMITE",_button,GUILayout.Height(34)))Say("Removed "+_plugin.Spawns.ClearLocalDynamite()+" tracked dynamite.");
            ...
            Card(new Rect(735,220,493,390),"☂  Item Storm",new Color(.35f,.68f,1f),delegate
            {
                ...
                if(!_itemStormRandom){GUILayout.Label("Selected: "+CurrentItemName(),_small);if(GUILayout.Button("CYCLE SELECTED ITEM",_button,GUILayout.Height(30)))CycleItem();}
                ...
                if(GUILayout.Button("CANCEL + CLEAR MY ITEMS",_button,GUILayout.Height(34)))Say("Removed "+_plugin.Spawns.ClearLocalItemStorm()+" tracked storm items.");
        ...
        private void DrawChaos(Rect area)
        {
            Card(new Rect(238,220,315,480),"◈  Chaos Controller",new Color(.63f,.42f,1f),delegate
            {
                ...
                GUILayout.BeginHorizontal(); if (GUILayout.Button("START", _success, GUILayout.Height(46))) { _plugin.Chaos.Start(); Say("Chaos started."); } if (GUILayout.Button("STOP", _danger, GUILayout.Height(46))) { _plugin.Chaos.Stop(); Say("Chaos stopped."); } GUILayout.EndHorizontal();
            ...
            Card(new Rect(565,220,330,480),"⏱  Chaos Combo Builder",new Color(.35f,.68f,1f),delegate
            {
                ...
                if(GUILayout.Button("CANCEL COMBO",_button,GUILayout.Height(34))){_plugin.Chaos.StopCombo();Say("Combo cancelled.");}
            ...
            Card(new Rect(907,220,321,480),"☠  Lobby Presets",new Color(1f,.42f,.2f),delegate
            {
                ...
                if(GUILayout.Button("CANCEL + CLEAR DYNAMITE",_button,GUILayout.Height(34)))Say("Removed "+_plugin.Spawns.ClearLocalDynamite()+" tracked dynamite.");
            });
        }
        ...
        {
            DrawPanel(new Rect(228, 762, 1012, 48), new Color(.04f, .1f, .16f, .96f), new Color(.16f, .32f, .45f, 1f), 1f);
            GUIStyle style = new GUIStyle(_small); style.alignment = TextAnchor.MiddleLeft; style.normal.textColor = Time.unscaledTime < _messageUntil ? new Color(.55f, 1f, .7f) : new Color(.55f, .64f, .75f); GUI.Label(new Rect(248, 770, 950, 32), _message, style);
        }

        private void Card(Rect rect, string title, Color accent, Action draw)
        {
            DrawPanel(rect, new Color(.018f, .055f, .09f, .95f), accent, 1f); GUIStyle ts = new GUIStyle(_cardTitle); ts.normal.textColor = accent; GUI.Label(new Rect(rect.x + 14, rect.y + 10, rect.width - 28, 30), title, ts);
        ...
        private bool CapabilityButton(string text, FeatureCapability capability, GUIStyle style, float height) { bool old=GUI.enabled;GUI.enabled=old&&_plugin.Capabilities.Available(capability);bool clicked=GUILayout.Button(text,style,GUILayout.Height(height));GUI.enabled=old;return clicked; }
        private void Unsupported(string name, string reason) { GUI.enabled = false; GUILayout.Button(name, _disabled, GUILayout.Height(32)); GUI.enabled = true; GUILayout.Label("⚠ " + reason, _small); }
        ...
        private void EnsureStyles()
        {
            ...
            _logo=Style(58,FontStyle.Bold,Color.white,TextAnchor.MiddleLeft); _title=Style(34,FontStyle.Bold,Color.white,TextAnchor.MiddleLeft); _subtitle=Style(14,FontStyle.Normal,new Color(.63f,.7f,.82f),TextAnchor.MiddleLeft);
            _label=Style(14,FontStyle.Normal,new Color(.9f,.94f,1f),TextAnchor.MiddleLeft); _small=Style(12,FontStyle.Normal,new Color(.58f,.67f,.78f),TextAnchor.MiddleLeft); _cardTitle=Style(19,FontStyle.Bold,Color.white,TextAnchor.MiddleLeft); _badge=Style(11,FontStyle.Bold,new Color(.86f,.9f,.98f),TextAnchor.MiddleLeft);
            _nav=ButtonStyle(16,new Color(.68f,.76f,.9f),new Color(.02f,.06f,.1f,.05f)); _nav.alignment=TextAnchor.MiddleLeft; _nav.padding=new RectOffset(18,8,0,0);
            _navSelected=ButtonStyle(16,Color.white,new Color(.08f,.27f,.58f,.92f)); _navSelected.alignment=TextAnchor.MiddleLeft; _navSelected.padding=new RectOffset(18,8,0,0);
            _button=ButtonStyle(13,Color.white,new Color(.09f,.18f,.29f,1f)); _danger=ButtonStyle(13,Color.white,new Color(.62f,.13f,.2f,1f)); _success=ButtonStyle(13,Color.white,new Color(.08f,.44f,.25f,1f)); _disabled=ButtonStyle(13,new Color(.4f,.46f,.54f),new Color(.08f,.1f,.13f,1f));
        }
        ...
        private GUIStyle ButtonStyle(int size, Color textColor, Color bg) { GUIStyle s=new GUIStyle(GUI.skin.button);s.fontSize=size;s.fontStyle=FontStyle.Bold;s.normal.textColor=textColor;s.hover.textColor=Color.white;s.active.textColor=Color.white;s.normal.background=Tint(bg);s.hover.background=Tint(new Color(bg.r+.05f,bg.g+.08f,bg.b+.12f,bg.a));s.active.background=Tint(new Color(bg.r+.02f,bg.g+.04f,bg.b+.06f,bg.a));s.padding=new RectOffset(10,10,6,6);return s; }
