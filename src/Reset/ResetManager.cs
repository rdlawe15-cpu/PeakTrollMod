...
namespace PeakTrollMod
{
    internal sealed class ResetManager
    {
        ...
        public void ResetAll() { TrollModPlugin plugin = TrollModPlugin.Instance; if (plugin != null && plugin.Chaos != null) { plugin.Chaos.Stop(); plugin.Chaos.StopCombo(); } if(plugin!=null&&plugin.Players!=null)_appearance.RestoreAll(plugin.Players.Entries); _actions.ResetKnownPlayers(); _mirages.ClearAll(); _spawns.ClearAll(); _audio.StopAll(); _audio.RestoreAllVoice(); _log.LogInfo("All reversible troll state cleaned up."); }
    }
}
